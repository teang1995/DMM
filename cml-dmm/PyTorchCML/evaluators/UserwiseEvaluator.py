...
class UserwiseEvaluator(BaseEvaluator):
    ...
    def __init__(
        ...
        self.metrics_names = [
            f"{name}@{k}" for name in score_function_dict.keys() for k in ks 
        ]
...
def hit(y_test_user: np.ndarray, y_hat_user: np.ndarray, k: int):
    pred_sort_indices = (-y_hat_user).argsort()[:k]
    topk_test_user = np.where(y_test_user == 1)[0]
    hit_ = any([x in topk_test_user for x in pred_sort_indices])
