...
-f with experiment.scenario="mosei" experiment.stage="train_model" model.infer_mixed=False model.dirichlet_params=[1.0,1.0,1.0] model.beta_params=[0.5,0.5] \
        experiment.model="mmix_${uni}_${all}_${mm}_${temperature}_0.5_${m2mix_type}" model.model="mmix_${uni}_${all}_${mm}_${temperature}_0.5_${m2mix_type}" model_train.epochs=40 experiment.seed=$seed model.mixup_type="replace" \
        model.single_mix=$uni model.all_mix=$all model.multi_mix=$mm model_train.batch_size=128 model_train.temperature=$temperature model.m2mix_type=$m2mix_type
