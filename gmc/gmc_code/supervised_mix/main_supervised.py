...
import torch
import sacred
import gmc_code.supervised_mix.ingredients.exp_ingredients as sacred_exp
import gmc_code.supervised_mix.ingredients.machine_ingredients as sacred_machine
import gmc_code.supervised_mix.ingredients.stage_specific.model_ingredients as sacred_model
import gmc_code.supervised_mix.ingredients.stage_specific.down_ingredients as sacred_down
from pytorch_lightning import Trainer, seed_everything
from pytorch_lightning.callbacks import ModelCheckpoint
from gmc_code.supervised_mix.modules.trainers.model_trainer import ModelLearner
from gmc_code.supervised_mix.modules.trainers.model_evaluation import ModelEvaluation
from gmc_code.supervised_mix.modules.callbacks import OnEndModelTrainingMosi, OnEndModelTrainingMosei
from gmc_code.supervised_mix.modules.sacred_loggers import SacredLogger

from gmc_code.supervised_mix.utils.general_utils import (
    setup_model,
    setup_data_module,
    load_model,
...
def train_model(_config, _run):
    ...
    trainer = Trainer(
        ...
        gradient_clip_val=0.8,                            #! 디폴트로 클리핑(0.8) 되있네..? 머냐;;
    )
...
def embedding_analysis(_config, _run):
    ...
    # get embedding
    partial, _ = affect_evaluator.get_emb(mod=obs_mod)
    ...
    # s_align = torch.mean(torch.sum(-(partial - joint)**2, dim=1)).item()
    #! modified alignment formula - relative alignment between pos and neg
    partial = partial.cpu()
    joint = joint.cpu()
