...
from gmc_code.DelaunayComponentAnalysis.DCA import DCA
...
class DCAEvaluator(LightningModule):
    ...
    def test_epoch_end(self, outputs):
        ...
        R = torch.cat([outputs[i][-1] for i in range(len(outputs))]).cpu().numpy()

        E_repr = []
        for mod in range(n_mod - 1):
            E = (
                torch.cat([outputs[i][mod] for i in range(len(outputs))])
                .cpu()
                .numpy()
            )
