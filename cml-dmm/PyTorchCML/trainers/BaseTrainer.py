...
from ..losses import BaseLoss, MinTripletLoss
        ...
        for ep in range(n_epoch):
            ...
            with tqdm(range(n_batch), total=n_batch) as pbar:
                for b in pbar:
                    ...
                    # compute loss
                    self.criterion = self.criterion_dict[self.args.loss]
                    loss = self.criterion(embeddings_dict, batch, self.column_names)
                    if self.args.loss_type == 'dmm':
                        self.criterion = self.criterion_dict[f'{self.args.loss}_dml']
                        mixed_loss = self.criterion(embeddings_dict, batch, self.column_names)
                        loss += (ep / (30 + ep)) * mixed_loss
            ...
            print(self.valid_scores)
            weight_file = f'{self.args.dataset}_{self.args.model}_{self.args.loss}_{ep}_{self.args.loss_type}.pt'
